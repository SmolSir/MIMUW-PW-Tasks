...
int compare_paths(const char* p1, const char* p2) {
    if (strlen(p1) > strlen(p2)) {
        if (strncmp(p1, p2, strlen(p2)) == 0) { // /a/a/ is more leftist than /a/
            return -1;
        }
        return strcmp(p1, p2); // p2 is not an initial substring of p1
    }
    else if (strlen(p1) == strlen(p2)) {
        return strcmp(p1, p2); // normal lexicographical comparison
    }
    else {
        if (strncmp(p1, p2, strlen(p1)) == 0) { // /a/ is less leftist than /a/a/
            return 1;
        }
        return strcmp(p1, p2); // p1 is not an initial substring of p2
    }
}
...
char* make_shared_path(const char* p_A, const char* p_B)
{
    const char* subpath = strchr(p_A + 1, '/');

    size_t shared_cnt = 1; // Include the initial '/'.
    while (subpath) {
        size_t name_length = subpath - (p_A + shared_cnt) + 1; // Include '/' following name.
        if (strncmp(p_A + shared_cnt, p_B + shared_cnt, name_length) != 0) {
            break;
        }
        shared_cnt += name_length;
        subpath = strchr(subpath + 1, '/');
    }

    char* result = safe_malloc(shared_cnt + 1); // Include terminating null character.
    strncpy(result, p_A, shared_cnt);
    result[shared_cnt] = '\0';
    return result;
}
