...
public class Kwadraty {
    ...
    private static final int WĄTKI = 4;
    ...
    public static void main(String[] args) {
        ExecutorService pulaLiczenia = Executors.newFixedThreadPool(WĄTKI);
        ExecutorService pulaPisania = Executors.newFixedThreadPool(WĄTKI);
        try {
            for (int i = 1; i <= LICZONE; ++i) {
                Callable<Integer> praca = new Liczący(i);
                Future<Integer> obietnica = pulaLiczenia.submit(praca);
                pulaPisania.submit(new Piszący(obietnica));
            }
        } finally {
            pulaLiczenia.shutdown();
            pulaPisania.shutdown();
        }
    }
