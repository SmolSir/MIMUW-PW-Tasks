...
import static przyklady05.Utils.*;
...
public class BankDatabase {
    ...
    public static CompletableFuture<List<Void>> robinHoodAsync(Database db) {
        ...
        return db.getAccountHoldersAsync()
                .thenComposeAsync(holders -> {
                    List<CompletableFuture<Entry<String, Integer>>> accounts_balances = new ArrayList<>();
                    for (String holder : holders) {
                        accounts_balances.add(CompletableFuture.supplyAsync(() -> Map.entry(holder, db.getAccountBalance(holder))));
                    }
                    return awaitAll(accounts_balances);
                })
                .thenApplyAsync(Utils::createMapFromEntries)
                .thenComposeAsync(accounts -> {

                    List<String> holders = accounts.keySet().stream().toList();
                    Optional<Entry<String, Integer>> maybeRichestHolder = findTheRichestHolder(accounts);
                    List<CompletableFuture<Void>> justice = new ArrayList<>();
                    ...
                    if (maybeRichestHolder.isPresent() && holders.size() > 1) {
                        ...
                        for (String holder : holders) {
                            justice.add(holder.equals(richestHolder) ? db.closeAccountAsync(holder) : db.addMoneyAsync(holder, moneyForEach));
                        }
                    }
                    return awaitAll(justice);
                });
    }
    ...
    public static void main(String[] args) {
        ...
        if (asyncDuration.compareTo(Duration.of(15, ChronoUnit.SECONDS)) > 0) {
