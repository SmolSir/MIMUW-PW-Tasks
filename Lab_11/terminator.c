...
int child()
{
    ...
    CHECK_ERRNO(sigaction(SIGINT, NULL, &action));
    action.sa_handler = handler;
    CHECK_ERRNO(sigaction(SIGINT, &action, NULL));
...
int parent(pid_t child_pid)
{
    ...
    CHECK_ERRNO(sigprocmask(SIG_BLOCK, &signals_to_wait_for, NULL));
    ...
    while (true) {
        ...
        if (info.si_signo == SIGCHLD) {
            printf("My child died so I will die too!\n");
            break;
        }
        ...
        if (kill(child_pid, SIGINT)) {
            printf("Failed to signal child with SIGINT\n");
        }
        ...
        if (ret == -1) // Timeout.
            ...
            if (kill(child_pid, SIGKILL)) {
                printf("Failed to signal child with SIGKILL\n");
            }
            break;
        }
    }
    // We won't wait for signals anymore, so we unblock default handlers, just in case.
    CHECK_ERRNO(sigprocmask(SIG_UNBLOCK, &signals_to_wait_for, NULL));
